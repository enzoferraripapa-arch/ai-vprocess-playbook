...
def check_blocked_files(paths: list[Path]) -> list[str]:
    errors: list[str] = []
    for path in paths:
        if path.suffix.lower() in BLOCKED_SUFFIXES:
            errors.append(f"blocked file type: {path.relative_to(ROOT)}")
    return errors
...
def read_text(path: Path) -> str | None:
    ...
    try:
        ...
        return f"<decode error: {path.relative_to(ROOT)}>"
...
def check_text(paths: list[Path]) -> list[str]:
    ...
    for path in paths:
        ...
        if text.startswith("<decode error:"):
            errors.append(text[1:-1])
            continue
        lowered = text.lower()
...
def main() -> int:
    paths = iter_files()
    errors = []
    errors.extend(check_blocked_files(paths))
    errors.extend(check_text(paths))
    errors.extend(check_json(paths))
    if errors:
        for error in errors:
            print(error, file=sys.stderr)
        return 1
    print("Public safety gate passed.")
    print("Checked: file types, UTF-8 text, private markers, secret patterns, JSON.")
    return 0
